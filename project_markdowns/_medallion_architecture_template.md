...
This section provides a comprehensive, production-grade Medallion architecture (Bronze → Silver → Gold) for the project `{{PREFIX}}`, including:
...
### Pipeline: `{{PREFIX}}_Master_Orchestration`
...
1. **Activity: Lookup_ExecutionParams**
   - Query `{{PREFIX}}_audit_ingest` for last successful run
...
2. **Activity: Copy_Source_A** (For each source in parallel)
   ...
   - Sink: `{{PREFIX}}_bro_source_a_raw` (ADLS gen2)
   - Format: Parquet (snappy compression)
   - Error handling: Retry 3x, then store error metadata
   - Post-copy: Execute "Validate_Bronze_A"

3. **Activity: Validate_Bronze_A**
   - Stored procedure call to `sp_validate_bronze_{{PREFIX}}_a(batch_id)`
   - Returns validation result set
   - If failures: Log to `{{PREFIX}}_audit_validation_failures`, alert, optionally fail pipeline

4. **Activity: Trigger_Spark_Silver_Job**
   - Type: Databricks Notebook Activity
   - Notebook path: `/Workspace/{{PREFIX}}/silver_transform`
   - Parameters: `batch_id`, `execution_date`
   - Cluster: `{{PREFIX}}-shared-compute` (auto-scale 2-8 nodes)

5. **Activity: Trigger_Spark_Gold_Job**
   - Notebook path: `/Workspace/{{PREFIX}}/gold_aggregate`
...
6. **Activity: Post_Load_Validations**
   ...
   - Store results in `{{PREFIX}}_gold_validation_log`
...
**Table Schema Example:** `{{PREFIX}}_bro_source_a_raw`

```sql
CREATE TABLE {{PREFIX}}_bro_source_a_raw (
...
spark = SparkSession.builder.appName("{{PREFIX}}_bronze_validate").getOrCreate()

# Load Bronze raw data
bro_df = spark.read.parquet(f"/mnt/adls/bronze/{{PREFIX}}_bro_source_a_raw/load_date={exec_date}")
...
for val_id, fail_count in validations.items():
    ...
    if fail_count > 0:
        ...
        spark.sql(f"""
        INSERT INTO {{PREFIX}}_audit_validation_results (validation_id, layer, fail_count, check_ts)
...
**Table Example:** `{{PREFIX}}_sil_transactions_curated`

```sql
CREATE TABLE {{PREFIX}}_sil_transactions_curated (
...
spark = SparkSession.builder.appName("{{PREFIX}}_silver_transform").getOrCreate()
...
acct_df = spark.read.table("{{PREFIX}}_dim_account")
...
sil_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("load_date").save(f"/mnt/adls/silver/{{PREFIX}}_sil_transactions_curated")
...
**Table Example:** `{{PREFIX}}_gld_fact_transactions`

```sql
CREATE TABLE {{PREFIX}}_gld_fact_transactions (
...
spark = SparkSession.builder.appName("{{PREFIX}}_gold_aggregate").getOrCreate()

# Load Silver data
sil_df = spark.read.table("{{PREFIX}}_sil_transactions_curated")
...
acct_df = spark.read.table("{{PREFIX}}_dim_account").select("account_key")
...
for val_id, count in validation_results.items():
    spark.sql(f"""
    INSERT INTO {{PREFIX}}_gold_validation_log (validation_id, layer, fail_count, check_ts)
    VALUES ('{val_id}', 'GOLD', {count}, current_timestamp())
    """)

# Write fact table
gld_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("date_key").save(f"/mnt/adls/gold/{{PREFIX}}_gld_fact_transactions")
