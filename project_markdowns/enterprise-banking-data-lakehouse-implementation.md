...
This detailed ER diagram template expands the ASCII ER stub into full entity descriptions, attribute lists, primary/foreign key definitions, and explicit relationships with cardinalities for the project prefix `enterprise_banking_data_lakehouse_implementation` (source file `enterprise-banking-data-lakehouse-implementation.md`).
...
**Entity: `enterprise_banking_data_lakehouse_implementation_raw_records`**
...
**Entity: `enterprise_banking_data_lakehouse_implementation_stg_<topic>`** (generic staging)
- Purpose: Parsed staging of a raw record specific to a topic (e.g. transactions, events).
- PK: `stg_id`
- Key attributes: `stg_id (PK)`, `record_key (FK -> enterprise_banking_data_lakehouse_implementation_raw_records.record_key)`, `parsed_fields...`, `ingest_ts`, `batch_id`, `load_status`.

**Entity: `enterprise_banking_data_lakehouse_implementation_dim_account`**
...
**Entity: `enterprise_banking_data_lakehouse_implementation_dim_customer`**
...
**Entity: `enterprise_banking_data_lakehouse_implementation_dim_time`**
- Purpose: Standard time dimension for reporting.
- PK: `date_key`
- Attributes: `date_key (PK)`, `date`, `year`, `quarter`, `month`, `day`, `weekday`, `is_business_day`.

**Entity: `enterprise_banking_data_lakehouse_implementation_dim_product`**
...
**Entity: `enterprise_banking_data_lakehouse_implementation_fact_transactions`**
...
- `enterprise_banking_data_lakehouse_implementation_fact_transactions.account_id` (M:1) -> `enterprise_banking_data_lakehouse_implementation_dim_account.account_id`
  - Cardinality: Many transactions map to one account.
- `enterprise_banking_data_lakehouse_implementation_fact_transactions.customer_id` (M:1) -> `enterprise_banking_data_lakehouse_implementation_dim_customer.customer_id`
  - Many transactions per customer; some transactions inferred to customer through account linkage.
- `enterprise_banking_data_lakehouse_implementation_fact_transactions.product_id` (M:1) -> `enterprise_banking_data_lakehouse_implementation_dim_product.product_id`
  - Many fact rows reference a single product row.
- `enterprise_banking_data_lakehouse_implementation_fact_transactions.posted_ts` (M:1) -> `enterprise_banking_data_lakehouse_implementation_dim_time.date_key` (via date_key derived from posted_ts)
  - Many transactions map to one date in the time dimension.
- `enterprise_banking_data_lakehouse_implementation_stg_<topic>.record_key` (M:1) -> `enterprise_banking_data_lakehouse_implementation_raw_records.record_key`
  - One staging row per raw record, after parsing.

Association tables / many-to-many patterns:
- If customers belong to segments or tags, model as `enterprise_banking_data_lakehouse_implementation_rel_customer_segment(customer_id FK, segment_id FK, assigned_ts)` with PK composite (`customer_id`, `segment_id`).

Detailed ER rules & integrity constraints:
- Use FK constraints where possible in the curated warehouse (e.g., `FOREIGN KEY (account_id) REFERENCES enterprise_banking_data_lakehouse_implementation_dim_account(account_id)`).
...
CREATE TABLE enterprise_banking_data_lakehouse_implementation_dim_customer (
...
CREATE TABLE enterprise_banking_data_lakehouse_implementation_fact_transactions (
  ...
  checksum STRING,
  FOREIGN KEY (account_id) REFERENCES enterprise_banking_data_lakehouse_implementation_dim_account(account_id),
  FOREIGN KEY (customer_id) REFERENCES enterprise_banking_data_lakehouse_implementation_dim_customer(customer_id)
);

ER Diagram (visual guidance):
- Center the `enterprise_banking_data_lakehouse_implementation_fact_transactions` as the hub with outward links to `dim_account`, `dim_customer`, `dim_product`, and `dim_time`.
- Secondary facts (e.g., `enterprise_banking_data_lakehouse_implementation_fact_settlements`) link back to `fact_transactions` via `transaction_id` where the settlement is child of a transaction (1:M relationship).
...
- For IoT/telemetry projects, add time-series entity `enterprise_banking_data_lakehouse_implementation_ts_measurements(device_id, measure_ts, metric, value)` with retention and rollup rules described.

This template is intended to be expanded with project-specific entities. The automation script will replace `enterprise_banking_data_lakehouse_implementation` and `enterprise-banking-data-lakehouse-implementation.md` and append the section to each markdown file that does not already contain a `## Detailed ER Diagram` marker.



# Complete Flow Diagram & Medallion Architecture with ADF, ADB, and PySpark

This section provides a comprehensive, production-grade Medallion architecture (Bronze → Silver → Gold) for the project `enterprise_banking_data_lakehouse_implementation`, including:
...
└────────┬──────┬──────────┬──────────┬──────────┬───────────────────────┘
   ...
   │  - enterprise_banking_data_lakehouse_implementation_bro_*  │    │   (Staging area)    │
   │  - Immutable raw     │    └─────────────────────┘
   │  - 20 Validations    │
   │  - Checksums & hash  │
   └──────────┬───────────┘
   ...
   │  - enterprise_banking_data_lakehouse_implementation_sil_*          │
   ...
   │  - enterprise_banking_data_lakehouse_implementation_gld_*          │
...
### Pipeline: `enterprise_banking_data_lakehouse_implementation_Master_Orchestration`
...
1. **Activity: Lookup_ExecutionParams**
   - Query `enterprise_banking_data_lakehouse_implementation_audit_ingest` for last successful run
...
2. **Activity: Copy_Source_A** (For each source in parallel)
   ...
   - Sink: `enterprise_banking_data_lakehouse_implementation_bro_source_a_raw` (ADLS gen2)
   - Format: Parquet (snappy compression)
   - Error handling: Retry 3x, then store error metadata
   - Post-copy: Execute "Validate_Bronze_A"

3. **Activity: Validate_Bronze_A**
   - Stored procedure call to `sp_validate_bronze_enterprise_banking_data_lakehouse_implementation_a(batch_id)`
   - Returns validation result set
   - If failures: Log to `enterprise_banking_data_lakehouse_implementation_audit_validation_failures`, alert, optionally fail pipeline

4. **Activity: Trigger_Spark_Silver_Job**
   - Type: Databricks Notebook Activity
   - Notebook path: `/Workspace/enterprise_banking_data_lakehouse_implementation/silver_transform`
   - Parameters: `batch_id`, `execution_date`
   - Cluster: `enterprise_banking_data_lakehouse_implementation-shared-compute` (auto-scale 2-8 nodes)

5. **Activity: Trigger_Spark_Gold_Job**
   - Notebook path: `/Workspace/enterprise_banking_data_lakehouse_implementation/gold_aggregate`
...
6. **Activity: Post_Load_Validations**
   ...
   - Store results in `enterprise_banking_data_lakehouse_implementation_gold_validation_log`
...
**Table Schema Example:** `enterprise_banking_data_lakehouse_implementation_bro_source_a_raw`

```sql
CREATE TABLE enterprise_banking_data_lakehouse_implementation_bro_source_a_raw (
...
spark = SparkSession.builder.appName("enterprise_banking_data_lakehouse_implementation_bronze_validate").getOrCreate()

# Load Bronze raw data
bro_df = spark.read.parquet(f"/mnt/adls/bronze/enterprise_banking_data_lakehouse_implementation_bro_source_a_raw/load_date={exec_date}")
...
for val_id, fail_count in validations.items():
    ...
    if fail_count > 0:
        ...
        spark.sql(f"""
        INSERT INTO enterprise_banking_data_lakehouse_implementation_audit_validation_results (validation_id, layer, fail_count, check_ts)
...
**Table Example:** `enterprise_banking_data_lakehouse_implementation_sil_transactions_curated`

```sql
CREATE TABLE enterprise_banking_data_lakehouse_implementation_sil_transactions_curated (
...
spark = SparkSession.builder.appName("enterprise_banking_data_lakehouse_implementation_silver_transform").getOrCreate()
...
acct_df = spark.read.table("enterprise_banking_data_lakehouse_implementation_dim_account")
...
sil_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("load_date").save(f"/mnt/adls/silver/enterprise_banking_data_lakehouse_implementation_sil_transactions_curated")
...
**Table Example:** `enterprise_banking_data_lakehouse_implementation_gld_fact_transactions`

```sql
CREATE TABLE enterprise_banking_data_lakehouse_implementation_gld_fact_transactions (
  transaction_id STRING PRIMARY KEY,
  account_key STRING,
  customer_key STRING,
...
spark = SparkSession.builder.appName("enterprise_banking_data_lakehouse_implementation_gold_aggregate").getOrCreate()

# Load Silver data
sil_df = spark.read.table("enterprise_banking_data_lakehouse_implementation_sil_transactions_curated")
...
acct_df = spark.read.table("enterprise_banking_data_lakehouse_implementation_dim_account").select("account_key")
...
for val_id, count in validation_results.items():
    spark.sql(f"""
    INSERT INTO enterprise_banking_data_lakehouse_implementation_gold_validation_log (validation_id, layer, fail_count, check_ts)
    VALUES ('{val_id}', 'GOLD', {count}, current_timestamp())
    """)

# Write fact table
gld_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("date_key").save(f"/mnt/adls/gold/enterprise_banking_data_lakehouse_implementation_gld_fact_transactions")
