...
This section provides a comprehensive, production-grade Medallion architecture (Bronze → Silver → Gold) for the project `iot_data_processing_for_predictive_maintenance`, including:
...
└────────┬──────┬──────────┬──────────┬──────────┬───────────────────────┘
   ...
   │  - iot_data_processing_for_predictive_maintenance_bro_*  │    │   (Staging area)    │
   │  - Immutable raw     │    └─────────────────────┘
   │  - 20 Validations    │
   │  - Checksums & hash  │
   └──────────┬───────────┘
   ...
   │  - iot_data_processing_for_predictive_maintenance_sil_*          │
   ...
   │  - iot_data_processing_for_predictive_maintenance_gld_*          │
...
### Pipeline: `iot_data_processing_for_predictive_maintenance_Master_Orchestration`
...
1. **Activity: Lookup_ExecutionParams**
   - Query `iot_data_processing_for_predictive_maintenance_audit_ingest` for last successful run
...
2. **Activity: Copy_Source_A** (For each source in parallel)
   ...
   - Sink: `iot_data_processing_for_predictive_maintenance_bro_source_a_raw` (ADLS gen2)
   - Format: Parquet (snappy compression)
   - Error handling: Retry 3x, then store error metadata
   - Post-copy: Execute "Validate_Bronze_A"

3. **Activity: Validate_Bronze_A**
   - Stored procedure call to `sp_validate_bronze_iot_data_processing_for_predictive_maintenance_a(batch_id)`
   - Returns validation result set
   - If failures: Log to `iot_data_processing_for_predictive_maintenance_audit_validation_failures`, alert, optionally fail pipeline

4. **Activity: Trigger_Spark_Silver_Job**
   - Type: Databricks Notebook Activity
   - Notebook path: `/Workspace/iot_data_processing_for_predictive_maintenance/silver_transform`
   - Parameters: `batch_id`, `execution_date`
   - Cluster: `iot_data_processing_for_predictive_maintenance-shared-compute` (auto-scale 2-8 nodes)

5. **Activity: Trigger_Spark_Gold_Job**
   - Notebook path: `/Workspace/iot_data_processing_for_predictive_maintenance/gold_aggregate`
...
6. **Activity: Post_Load_Validations**
   ...
   - Store results in `iot_data_processing_for_predictive_maintenance_gold_validation_log`
...
**Table Schema Example:** `iot_data_processing_for_predictive_maintenance_bro_source_a_raw`

```sql
CREATE TABLE iot_data_processing_for_predictive_maintenance_bro_source_a_raw (
...
spark = SparkSession.builder.appName("iot_data_processing_for_predictive_maintenance_bronze_validate").getOrCreate()

# Load Bronze raw data
bro_df = spark.read.parquet(f"/mnt/adls/bronze/iot_data_processing_for_predictive_maintenance_bro_source_a_raw/load_date={exec_date}")
...
for val_id, fail_count in validations.items():
    ...
    if fail_count > 0:
        ...
        spark.sql(f"""
        INSERT INTO iot_data_processing_for_predictive_maintenance_audit_validation_results (validation_id, layer, fail_count, check_ts)
...
**Table Example:** `iot_data_processing_for_predictive_maintenance_sil_transactions_curated`

```sql
CREATE TABLE iot_data_processing_for_predictive_maintenance_sil_transactions_curated (
...
spark = SparkSession.builder.appName("iot_data_processing_for_predictive_maintenance_silver_transform").getOrCreate()
...
acct_df = spark.read.table("iot_data_processing_for_predictive_maintenance_dim_account")
...
sil_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("load_date").save(f"/mnt/adls/silver/iot_data_processing_for_predictive_maintenance_sil_transactions_curated")
...
**Table Example:** `iot_data_processing_for_predictive_maintenance_gld_fact_transactions`

```sql
CREATE TABLE iot_data_processing_for_predictive_maintenance_gld_fact_transactions (
  transaction_id STRING PRIMARY KEY,
  account_key STRING,
  customer_key STRING,
...
spark = SparkSession.builder.appName("iot_data_processing_for_predictive_maintenance_gold_aggregate").getOrCreate()

# Load Silver data
sil_df = spark.read.table("iot_data_processing_for_predictive_maintenance_sil_transactions_curated")
...
acct_df = spark.read.table("iot_data_processing_for_predictive_maintenance_dim_account").select("account_key")
...
for val_id, count in validation_results.items():
    spark.sql(f"""
    INSERT INTO iot_data_processing_for_predictive_maintenance_gold_validation_log (validation_id, layer, fail_count, check_ts)
    VALUES ('{val_id}', 'GOLD', {count}, current_timestamp())
    """)

# Write fact table
gld_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("date_key").save(f"/mnt/adls/gold/iot_data_processing_for_predictive_maintenance_gld_fact_transactions")
