...
This detailed ER diagram template expands the ASCII ER stub into full entity descriptions, attribute lists, primary/foreign key definitions, and explicit relationships with cardinalities for the project prefix `customer_behavioral_and_360_analytics` (source file `customer-behavioral-and-360-analytics.md`).
...
**Entity: `customer_behavioral_and_360_analytics_raw_records`**
...
**Entity: `customer_behavioral_and_360_analytics_stg_<topic>`** (generic staging)
- Purpose: Parsed staging of a raw record specific to a topic (e.g. transactions, events).
- PK: `stg_id`
- Key attributes: `stg_id (PK)`, `record_key (FK -> customer_behavioral_and_360_analytics_raw_records.record_key)`, `parsed_fields...`, `ingest_ts`, `batch_id`, `load_status`.

**Entity: `customer_behavioral_and_360_analytics_dim_account`**
...
**Entity: `customer_behavioral_and_360_analytics_dim_customer`**
...
**Entity: `customer_behavioral_and_360_analytics_fact_transactions`**
...
- `customer_behavioral_and_360_analytics_fact_transactions.account_id` (M:1) -> `customer_behavioral_and_360_analytics_dim_account.account_id`
  - Cardinality: Many transactions map to one account.
- `customer_behavioral_and_360_analytics_fact_transactions.customer_id` (M:1) -> `customer_behavioral_and_360_analytics_dim_customer.customer_id`
  - Many transactions per customer; some transactions inferred to customer through account linkage.
- `customer_behavioral_and_360_analytics_fact_transactions.product_id` (M:1) -> `customer_behavioral_and_360_analytics_dim_product.product_id`
  - Many fact rows reference a single product row.
- `customer_behavioral_and_360_analytics_fact_transactions.posted_ts` (M:1) -> `customer_behavioral_and_360_analytics_dim_time.date_key` (via date_key derived from posted_ts)
  - Many transactions map to one date in the time dimension.
- `customer_behavioral_and_360_analytics_stg_<topic>.record_key` (M:1) -> `customer_behavioral_and_360_analytics_raw_records.record_key`
  - One staging row per raw record, after parsing.

Association tables / many-to-many patterns:
- If customers belong to segments or tags, model as `customer_behavioral_and_360_analytics_rel_customer_segment(customer_id FK, segment_id FK, assigned_ts)` with PK composite (`customer_id`, `segment_id`).

Detailed ER rules & integrity constraints:
- Use FK constraints where possible in the curated warehouse (e.g., `FOREIGN KEY (account_id) REFERENCES customer_behavioral_and_360_analytics_dim_account(account_id)`).
...
CREATE TABLE customer_behavioral_and_360_analytics_dim_customer (
...
CREATE TABLE customer_behavioral_and_360_analytics_fact_transactions (
  ...
  checksum STRING,
  FOREIGN KEY (account_id) REFERENCES customer_behavioral_and_360_analytics_dim_account(account_id),
  FOREIGN KEY (customer_id) REFERENCES customer_behavioral_and_360_analytics_dim_customer(customer_id)
);

ER Diagram (visual guidance):
- Center the `customer_behavioral_and_360_analytics_fact_transactions` as the hub with outward links to `dim_account`, `dim_customer`, `dim_product`, and `dim_time`.
- Secondary facts (e.g., `customer_behavioral_and_360_analytics_fact_settlements`) link back to `fact_transactions` via `transaction_id` where the settlement is child of a transaction (1:M relationship).
...
- For IoT/telemetry projects, add time-series entity `customer_behavioral_and_360_analytics_ts_measurements(device_id, measure_ts, metric, value)` with retention and rollup rules described.

This template is intended to be expanded with project-specific entities. The automation script will replace `customer_behavioral_and_360_analytics` and `customer-behavioral-and-360-analytics.md` and append the section to each markdown file that does not already contain a `## Detailed ER Diagram` marker.



# Complete Flow Diagram & Medallion Architecture with ADF, ADB, and PySpark

This section provides a comprehensive, production-grade Medallion architecture (Bronze → Silver → Gold) for the project `customer_behavioral_and_360_analytics`, including:
...
└────────┬──────┬──────────┬──────────┬──────────┬───────────────────────┘
   ...
   │  - customer_behavioral_and_360_analytics_bro_*  │    │   (Staging area)    │
   │  - Immutable raw     │    └─────────────────────┘
   │  - 20 Validations    │
   │  - Checksums & hash  │
   └──────────┬───────────┘
   ...
   │  - customer_behavioral_and_360_analytics_sil_*          │
   ...
   │  - customer_behavioral_and_360_analytics_gld_*          │
...
### Pipeline: `customer_behavioral_and_360_analytics_Master_Orchestration`
...
1. **Activity: Lookup_ExecutionParams**
   - Query `customer_behavioral_and_360_analytics_audit_ingest` for last successful run
...
2. **Activity: Copy_Source_A** (For each source in parallel)
   ...
   - Sink: `customer_behavioral_and_360_analytics_bro_source_a_raw` (ADLS gen2)
   - Format: Parquet (snappy compression)
   - Error handling: Retry 3x, then store error metadata
   - Post-copy: Execute "Validate_Bronze_A"

3. **Activity: Validate_Bronze_A**
   - Stored procedure call to `sp_validate_bronze_customer_behavioral_and_360_analytics_a(batch_id)`
   - Returns validation result set
   - If failures: Log to `customer_behavioral_and_360_analytics_audit_validation_failures`, alert, optionally fail pipeline

4. **Activity: Trigger_Spark_Silver_Job**
   - Type: Databricks Notebook Activity
   - Notebook path: `/Workspace/customer_behavioral_and_360_analytics/silver_transform`
   - Parameters: `batch_id`, `execution_date`
   - Cluster: `customer_behavioral_and_360_analytics-shared-compute` (auto-scale 2-8 nodes)

5. **Activity: Trigger_Spark_Gold_Job**
   - Notebook path: `/Workspace/customer_behavioral_and_360_analytics/gold_aggregate`
...
6. **Activity: Post_Load_Validations**
   ...
   - Store results in `customer_behavioral_and_360_analytics_gold_validation_log`
...
**Table Schema Example:** `customer_behavioral_and_360_analytics_bro_source_a_raw`

```sql
CREATE TABLE customer_behavioral_and_360_analytics_bro_source_a_raw (
...
spark = SparkSession.builder.appName("customer_behavioral_and_360_analytics_bronze_validate").getOrCreate()

# Load Bronze raw data
bro_df = spark.read.parquet(f"/mnt/adls/bronze/customer_behavioral_and_360_analytics_bro_source_a_raw/load_date={exec_date}")
...
for val_id, fail_count in validations.items():
    ...
    if fail_count > 0:
        ...
        spark.sql(f"""
        INSERT INTO customer_behavioral_and_360_analytics_audit_validation_results (validation_id, layer, fail_count, check_ts)
...
**Table Example:** `customer_behavioral_and_360_analytics_sil_transactions_curated`

```sql
CREATE TABLE customer_behavioral_and_360_analytics_sil_transactions_curated (
...
spark = SparkSession.builder.appName("customer_behavioral_and_360_analytics_silver_transform").getOrCreate()
...
acct_df = spark.read.table("customer_behavioral_and_360_analytics_dim_account")
...
sil_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("load_date").save(f"/mnt/adls/silver/customer_behavioral_and_360_analytics_sil_transactions_curated")
...
**Table Example:** `customer_behavioral_and_360_analytics_gld_fact_transactions`

```sql
CREATE TABLE customer_behavioral_and_360_analytics_gld_fact_transactions (
  transaction_id STRING PRIMARY KEY,
  account_key STRING,
  customer_key STRING,
...
spark = SparkSession.builder.appName("customer_behavioral_and_360_analytics_gold_aggregate").getOrCreate()

# Load Silver data
sil_df = spark.read.table("customer_behavioral_and_360_analytics_sil_transactions_curated")
...
acct_df = spark.read.table("customer_behavioral_and_360_analytics_dim_account").select("account_key")
...
for val_id, count in validation_results.items():
    spark.sql(f"""
    INSERT INTO customer_behavioral_and_360_analytics_gold_validation_log (validation_id, layer, fail_count, check_ts)
    VALUES ('{val_id}', 'GOLD', {count}, current_timestamp())
    """)

# Write fact table
gld_df.write.format("delta").mode("append").option("mergeSchema", "true") \
    .partitionBy("date_key").save(f"/mnt/adls/gold/customer_behavioral_and_360_analytics_gld_fact_transactions")
