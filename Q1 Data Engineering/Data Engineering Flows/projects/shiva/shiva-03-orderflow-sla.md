...
> **Reference:** Adapted from [https://github.com/Analytics6/Azure-Data-Engineering-Notes](https://github.com/Analytics6/Azure-Data-Engineering-Notes) — `09-Enterprise-Data-Governance-Observability-Platform.md, 12-Supply-Chain-Optimization-Dashboard.md`


## Project Overview and Business Context

Fulfillment centers require operational visibility into order lifecycle (placed → picked → shipped → delivered) with SLA thresholds by priority tier and carrier. Combines OLTP snapshots and event streams into Gold SLA dashboards and proactive alerts.
...
```mermaid
flowchart TB
    OMS[Order Management SQL] --> ADF
    WMS[WMS Events] --> EH
    EH --> ADB
    ADF --> Bronze
    ADB --> Silver --> Gold
    Gold --> PBI[Ops Dashboard]
    Gold --> Logic[Logic App Alerts]
```
...
### Bronze
- OMS orders/shipments incremental; WMS scan events stream
- `bronze/oms/`, `bronze/wms/events/`

### Silver
- `silver.fact_order_milestone` — unified timeline per `order_id`
- SLA clock start/stop rules applied

### Gold
- `gold.order_sla_status`, `gold.carrier_performance_daily`
- Breach flag: `is_sla_missed` for Logic App
...
| Pipeline | Purpose |
|----------|---------|
| `PL_OF_Bronze_OMS_15min` | Near-real-time OMS delta |
| `PL_OF_Silver_Gold` | Milestone build |
| `PL_OF_SLA_Alert` | Lookup breaches → Web POST |

**Triggers:** 15-min schedule; tumbling window for SLA rollup.

**If Condition:** `breach_count > 0` → notify.

---

## ADB Notebooks

`NB_Stream_WMS_Scans`, `NB_Silver_Order_Milestones`, `NB_Gold_SLA_Calc`.

**Cluster:** Micro-batch streaming 4–8 workers; batch Gold every 15 min via `foreachBatch`.

---

## Data Flow

OMS + WMS → Bronze → Silver milestones → Gold SLA → Power BI + alerts.

---

## Support Elements

Runbook `RB-OF-001` late shipment backlog; watermark on `last_modified_utc`.

---

## Checklist

SLA rules documented, breach alerts tested, carrier dims conformed.
