...
> **Reference:** Adapted from [https://github.com/Analytics6/Azure-Data-Engineering-Notes](https://github.com/Analytics6/Azure-Data-Engineering-Notes) — `04-Retail-E-commerce-Data-Platform.md`


## Project Overview and Business Context

E-commerce platform streams click, cart, checkout, and payment events for real-time funnel analytics and personalization triggers. Event Hubs + Databricks Structured Streaming with Lambda architecture (speed + batch layers).
...
```mermaid
flowchart LR
    Web[Web/App SDK] --> EH[Event Hubs]
    EH --> Stream[Databricks Streaming]
    EH --> Capture --> ADF
    Stream --> SilverRT[Silver RT]
    ADF --> SilverBatch[Silver Batch]
    SilverRT --> Gold
    SilverBatch --> Gold
    Gold --> Redis[Azure Cache / API]
```
...
### Bronze
- Kafka/Event Hubs topic `ecom-events`; Capture to `bronze/events/raw/`
- Schema: `event_type, session_id, user_id, sku, timestamp, properties MAP`

### Silver
- Streaming: sessionization, 15-min watermark, dedup `event_id`
- `silver.fact_events_conformed` (streaming + batch merge)

### Gold
- `gold.funnel_metrics_5min`, `gold.realtime_revenue_by_category`
- Push to Redis for "trending now" API

---

## ADF Orchestration

`PL_SP_Capture_Compact` (hourly), `PL_SP_Gold_Batch_Reconcile` (daily), `PL_SP_Stream_Health` (5 min Web → Jobs API).

**Triggers:** Schedule + Event Grid on Capture blob.

---

## ADB Streaming

`NB_Stream_Ingest_Ecom` — `readStream.format("kafka")`, checkpoint `checkpoints/ecom_v3/`, trigger 10s.

`NB_Stream_Silver_Sessionize`, `NB_Gold_Funnel_5min` (foreachBatch).

**Cluster:** Streaming 8–32 autoscale; separate dev checkpoint path.

---

## Security & Monitoring

MI; user_id hashed in Silver. Alerts: lag > 3 min, Gold 5min table stale.

---

## Naming

`ehns-streampulse-prod`, `adb-shiva-stream-prod`. Phases: SDK → stream → Gold API → load test.
