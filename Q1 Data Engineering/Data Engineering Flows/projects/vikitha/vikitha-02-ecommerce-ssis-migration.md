...
> **Reference:** Adapted from [https://github.com/Analytics6/Azure-Data-Engineering-Notes](https://github.com/Analytics6/Azure-Data-Engineering-Notes) — `15-Retail-Migration-OnPrem-to-Azure.md, Data_Platform_Development_Global_Organisation.md`


## Project Overview and Business Context

Legacy SSIS packages (50+ projects) load e-commerce staging and marts on SQL Server. This migration refactors ETL to ADF orchestration + Databricks PySpark/Delta while preserving business logic for web analytics, cart abandonment, and campaign attribution.

**Outcomes:** Decommission SSIS, reduce runtime 40%, enable version-controlled Spark code.
...
```mermaid
flowchart TB
    SSIS[Legacy SSIS / SQL Server] -.->|assess| MAP[Package Mapping]
    SQL[(SQL Server On-Prem)] --> ADF
    ADF --> Bronze
    Bronze --> ADB --> Silver --> Gold
    Gold --> Synapse[Synapse Serverless]
```
...
### Bronze
- SSIS sources: `stg_orders`, `stg_clicks`, `stg_campaigns` via ADF Copy incremental
- Path: `bronze/ecommerce/{table}/`

### Silver
- Replace SSIS Data Flow transforms: lookups → Spark joins; slowly changing → `merge` SCD2 on `dim_customer`
- `silver.web_sessions`, `silver.orders`

### Gold
- `gold.fact_daily_sales`, `gold.fact_cart_abandonment`, `gold.campaign_attribution` (matches legacy mart names for BI cutover)

---

## ADF Orchestration

| Pipeline | SSIS Replacement |
|----------|------------------|
| `PL_ECOM_Bronze_Incremental` | `pkg_Load_Staging_Orders.dtsx` |
| `PL_ECOM_Silver_Gold` | `pkg_Build_And_Load_Marts.dtsx` |
| `PL_ECOM_Validation` | `pkg_Reject_Audit.dtsx` |

**Activities:** Copy (incremental), Execute Pipeline (orchestration master), Databricks Notebook, Stored Procedure (watermark), If Condition (reject rate < 1%).

**Triggers:** Tumbling window hourly for clicks; daily 02:00 for orders.

**SHIR** required for on-prem SQL until cutover.
...
```
/SSISMigration/ECommerce/
├── Mapping/SSIS_Package_Registry (CSV)
├── 02_Silver/NB_Silver_Web_Sessions, NB_SCD2_Customer
├── 03_Gold/NB_Gold_Campaign_Attribution
└── 04_Validation/NB_SSIS_Parity_Check
```

**Cluster:** 8–12 workers for daily; Unity Catalog `ecom_mig_{env}`. **Validation notebook** compares row counts vs. legacy SQL snapshots.

---

## Migration Approach

1. Inventory SSIS packages → complexity score  
2. Parallel run (SSIS vs. Azure) for 2 weeks  
3. Refactor transforms in Silver (not lift-shift SSIS expressions)  
4. Cutover: redirect Power BI to Synapse external tables on Gold Delta  

---

## Security, Monitoring, Checklist

Key Vault, MSI, Purview PII tags on email. ADF failure alerts; reconciliation dashboard. Checklist: all P1 packages mapped, parity signed-off, SSIS disabled.
