...
> **Reference:** Adapted from [https://github.com/Analytics6/Azure-Data-Engineering-Notes](https://github.com/Analytics6/Azure-Data-Engineering-Notes) — `01-Supply-Chain-Analytics-Platform.md, 12-Supply-Chain-Optimization-Dashboard.md`


## Project Overview and Business Context

**Differentiation vs. Manasa/Roja:** Pavani’s design centers on **multi-echelon supply chain** (DC → store → vendor ASN) with **Synapse Serverless** as the primary consumption layer for vendor portal and ops teams—not Unity Catalog lakehouse-first.

Monitor stock levels, inbound ASNs, and out-of-stock risk in near real time across 2,000+ stores.
...
```mermaid
flowchart TB
    POS[POS Sales] --> EH[Event Hubs]
    WMS[WMS Inventory] --> ADF
    Vendors[Vendor EDI/ASN] --> ADF
    EH --> ADB[Databricks Streaming]
    ADF --> Bronze
    ADB --> Silver --> Gold
    Gold --> Synapse[Synapse Serverless]
    Gold --> Alerts[Monitor Alerts]
```
...
### Bronze
- POS inventory deltas (stream), WMS nightly snapshots, vendor ASN CSV/EDI
- Paths: `bronze/pos/inventory_events/`, `bronze/wms/stock/`, `bronze/vendor/asn/`

### Silver
- `silver.inventory_position` — current qty by `store_id, sku` (streaming upserts)
- `silver.inbound_shipments` — ASN parsed

### Gold
- `gold.stockout_risk_score`, `gold.supply_chain_sla` (vendor fill rate)
- Synapse external tables over Gold Delta
...
| Pipeline | Purpose |
|----------|---------|
| `PL_SC_Stream_Health` | 5-min Databricks job check |
| `PL_SC_Bronze_WMS` | Nightly full + hourly delta |
| `PL_SC_Bronze_Vendor_ASN` | ForEach vendor file |
| `PL_SC_Gold_Refresh` | Batch complement to stream |

**Triggers:** Hourly WMS; event on blob arrival for ASN. **Linked services:** ADLS, Databricks, SFTP vendor.
...
```
/SupplyChain/Pavani/
├── NB_Stream_POS_Inventory
├── NB_Silver_Inventory_Position
├── NB_Gold_Stockout_Risk
└── NB_Vendor_ASN_Parse
```

**Streaming:** checkpoint `checkpoints/sc_inv_v1/`, watermark 10 min. **Cluster:** Streaming 8 workers; batch 4 workers.

---

## End-to-End

POS/WMS/Vendor → Bronze → Silver positions → Gold risk → Synapse + alerts.

---

## Security & Phases

Key Vault EDI credentials; MSI. Phases: WMS batch → POS stream → vendor ASN → Gold SLA alerts.
