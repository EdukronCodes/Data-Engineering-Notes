...
Classic on-prem SQL Server DW cutover to Synapse dedicated SQL + ADLS medallion.

**Devi focus:** Synapse dedicated pool external tables (not Databricks SQL warehouse).
...
| Pipeline | Purpose |
|----------|---------|
| `PL_DEVI_MIG_Bronze` | See ADF orchestration below |
| `PL_DEVI_MIG_Silver_Gold` | See ADF orchestration below |
| `PL_DEVI_MIG_Validation` | See ADF orchestration below |
| `PL_DEVI_Cutover` | See ADF orchestration below |
...
```
/Devi/Migration/NB_Silver_Conform
NB_Gold_Star_Schema
NB_Reconcile
```
