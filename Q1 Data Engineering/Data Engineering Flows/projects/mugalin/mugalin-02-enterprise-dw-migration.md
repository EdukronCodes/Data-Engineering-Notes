...
Ten-year on-premises retail enterprise data warehouse (sales, inventory, customer, finance, supply chain) migrates to a **Databricks lakehouse** with Unity Catalog governance. Analysts transition from SSRS/Cognos on SQL Server to **Databricks SQL warehouses** and Power BI on Gold Delta.

**Outcomes:** Retire on-prem DW within 6 months, 50% cost reduction, single conformed model for 45+ marts.
...
flowchart TB
    ...
    SHIR[Self-Hosted IR] --> ADF[Azure Data Factory]
    DW --> SHIR
    ERP --> SHIR
    ADF --> Bronze[(Bronze Delta)]
    Bronze --> ADB[Databricks]
    ADB --> Silver --> Gold
    Gold --> DBSQL[Databricks SQL WH]
    Gold --> PBI[Power BI]
    UC[Unity Catalog] --> Gold
```
...
ADF, Self-Hosted IR, ADLS Gen2, Azure Databricks, Unity Catalog, Delta Lake, Databricks SQL Warehouse, Key Vault, Azure Purview, Azure DevOps (CI/CD), Power BI.
...
| Item | Detail |
|------|--------|
| **Sources** | SQL Server DW (all schemas), supplemental ERP flat files |
| **Ingestion** | ADF metadata-driven Copy; watermark `ETL_UPDATED_DT` |
| **Path** | `abfss://bronze@stmugalinent{env}.dfs.core.windows.net/{source}/{table}/` |
| **Format** | Parquet → promoted to Delta via notebook |
| **Schema** | Source-faithful + `_ingest_batch_id`, `_ingest_ts` |

### Silver Layer

| Item | Detail |
|------|--------|
| **Purpose** | Enterprise bus matrix — conformed dimensions across domains |
| **Entities** | `silver.dim_customer`, `silver.dim_product`, `silver.dim_store`, `silver.fact_sales`, `silver.fact_inventory` |
| **SCD** | Type 2 on customer and product; Type 1 on store attributes |
| **DQ** | Null rates, FK integrity, duplicate PK checks |

### Gold Layer

| Item | Detail |
|------|--------|
| **Marts** | 45+ tables mirroring legacy names (`dm_sales_daily`, `dm_inventory_weekly`, etc.) |
| **Path** | `{catalog}.gold.{mart_table}` |
| **Serving** | Databricks SQL `wh_analyst_prod` (Medium, serverless); Power BI Direct Lake |
...
| Pipeline | Purpose |
|----------|---------|
| `PL_ENT_Metadata_Driven_Bronze` | ForEach table from `control.source_registry` |
| `PL_ENT_Silver_Domain_Sales` | Execute Pipeline per domain |
| `PL_ENT_Gold_Marts` | Parallel Gold loads by subject area |
| `PL_ENT_Validation` | Row count + aggregate parity vs. on-prem |
| `PL_ENT_Cutover` | Final incremental + disable on-prem jobs |

### Linked Services

`LS_SQL_DW_OnPrem`, `LS_ADLS`, `LS_Databricks`, `LS_KeyVault` — all SHIR/SQL auth via Key Vault.

### Activities

Copy → Databricks (Bronze to Delta) → Databricks Silver → Databricks Gold → If Validation Pass → Update watermark.

### Triggers

- `TR_ENT_Bronze_Nightly` — 22:00
- `TR_ENT_Validation` — tumbling window 6h during parallel run
- `TR_ENT_Cutover` — manual approval gate

### Parameters / Environments

`g_catalog`, `g_storage`, `g_subject_area`, `g_business_date` — set per dev/test/prod ARM parameters.
...
```
/EnterpriseDW/
├── 00_Config/NB_Source_Registry_Seed
├── 01_Bronze/NB_Promote_Parquet_To_Delta
├── 02_Silver/
│   ├── Sales/NB_Silver_Fact_Sales
│   ├── Inventory/NB_Silver_Fact_Inventory
│   └── Customer/NB_SCD2_Customer
├── 03_Gold/
│   ├── NB_Gold_dm_sales_daily
│   └── NB_Gold_dm_inventory_weekly
└── 04_Validation/NB_Parity_OnPrem_vs_Gold
```

**Cluster:** Initial load 32 workers `Standard_DS5_v2`; incremental 8 autoscale. **Unity Catalog:** `mugalin_ent_{env}` with row filters on finance tables.

---

## Migration Methodology

1. **Assessment** — DMA, bus matrix, 400-table inventory, P1/P2/P3 prioritization  
2. **Bronze** — Lift-shift extracts with minimal logic change  
3. **Silver/Gold** — Refactor stored procedures to Spark/Delta merge  
4. **Validation** — Automated parity for 30 critical marts  
5. **Cutover** — Weekend freeze; rollback script documented  

---

## Security & Monitoring

CMK on ADLS; UC grants by AD group; Purview scan on Bronze. ADF failure alerts; daily parity report to distribution list.

---

## Sample Naming

`adf-mugalin-ent-prod`, `adb-mugalin-ent-prod`, `stmugalinentprod`, `wh-analyst-prod`.

---

## Implementation Checklist

- [ ] Bus matrix signed by data governance
- [ ] P1 tables 100% parity for 14 consecutive days
- [ ] Analyst SQL warehouse UAT complete
- [ ] On-prem SQL Agent jobs disabled post-cutover
