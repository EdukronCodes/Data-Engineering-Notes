...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_02` starts `PL_AIRLINE_02`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_02` -> `NB_Silver_flight_curated_02` -> `NB_Gold_ops_kpi_02`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_03` starts `PL_AIRLINE_03`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_03` -> `NB_Silver_flight_curated_03` -> `NB_Gold_ops_kpi_03`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_04` starts `PL_AIRLINE_04`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_04` -> `NB_Silver_flight_curated_04` -> `NB_Gold_ops_kpi_04`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
```mermaid
flowchart LR
    TRG[ADF Trigger 04] --> PIPE[ADF Pipeline PL_AIRLINE_04]
    PIPE --> COPY[Copy/Landing to Bronze]
    COPY --> BR[(abfss://bronze@stairline{env}/flight_event/)]
    BR --> NB1[ADB NB_Bronze_flight_event_04]
    NB1 --> SV[(abfss://silver@stairline{env}/flight_curated/)]
    SV --> NB2[ADB NB_Silver_flight_curated_04]
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_05` starts `PL_AIRLINE_05`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_05` -> `NB_Silver_flight_curated_05` -> `NB_Gold_ops_kpi_05`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_06` starts `PL_AIRLINE_06`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_06` -> `NB_Silver_flight_curated_06` -> `NB_Gold_ops_kpi_06`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
```mermaid
flowchart LR
    TRG[ADF Trigger 06] --> PIPE[ADF Pipeline PL_AIRLINE_06]
    PIPE --> COPY[Copy/Landing to Bronze]
    COPY --> BR[(abfss://bronze@stairline{env}/flight_event/)]
    BR --> NB1[ADB NB_Bronze_flight_event_06]
    NB1 --> SV[(abfss://silver@stairline{env}/flight_curated/)]
    SV --> NB2[ADB NB_Silver_flight_curated_06]
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_07` starts `PL_AIRLINE_07`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_07` -> `NB_Silver_flight_curated_07` -> `NB_Gold_ops_kpi_07`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_08` starts `PL_AIRLINE_08`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_08` -> `NB_Silver_flight_curated_08` -> `NB_Gold_ops_kpi_08`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
```mermaid
flowchart LR
    TRG[ADF Trigger 08] --> PIPE[ADF Pipeline PL_AIRLINE_08]
    PIPE --> COPY[Copy/Landing to Bronze]
    COPY --> BR[(abfss://bronze@stairline{env}/flight_event/)]
    BR --> NB1[ADB NB_Bronze_flight_event_08]
    NB1 --> SV[(abfss://silver@stairline{env}/flight_curated/)]
    SV --> NB2[ADB NB_Silver_flight_curated_08]
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_09` starts `PL_AIRLINE_09`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_09` -> `NB_Silver_flight_curated_09` -> `NB_Gold_ops_kpi_09`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
- **Purpose:** Drive `airline` data from source to governed Gold outputs with observable checkpoints. Includes dependency gating and deterministic daily windows.
- **Trigger/Orchestration in ADF:** `TR_AIRLINE_10` starts `PL_AIRLINE_10`; Lookup -> IfCondition -> Copy -> Databricks activity chain with retry policy `(count=3, interval=120s)`.
- **Databricks notebook(s):** `NB_Bronze_flight_event_10` -> `NB_Silver_flight_curated_10` -> `NB_Gold_ops_kpi_10`.
- **Medallion transitions:** Bronze (`flight_event` raw + audit columns) -> Silver (`flight_curated` standardized, deduped, DQ-tagged) -> Gold (`ops_kpi` business serving model).
- **Inputs/Outputs and storage:** Input from `abfss://landing@stairline{env}/flight_event/`; writes to `abfss://bronze@stairline{env}/flight_event/`, `abfss://silver@stairline{env}/flight_curated/`, `abfss://gold@stairline{env}/ops_kpi/`.
- **Monitoring/Retry/Error handling:** ADF activity run IDs stamped into Delta audit table; failed notebooks route to `PL_AIRLINE_INCIDENT` with Logic App/Pager alert and rerun token.
...
```mermaid
flowchart LR
    TRG[ADF Trigger 10] --> PIPE[ADF Pipeline PL_AIRLINE_10]
    PIPE --> COPY[Copy/Landing to Bronze]
    COPY --> BR[(abfss://bronze@stairline{env}/flight_event/)]
    BR --> NB1[ADB NB_Bronze_flight_event_10]
    NB1 --> SV[(abfss://silver@stairline{env}/flight_curated/)]
    SV --> NB2[ADB NB_Silver_flight_curated_10]
...
```adf
@concat('abfss://landing@stairline', pipeline().globalParameters.g_env, '.dfs.core.windows.net/flight_event/', formatDateTime(pipeline().parameters.load_date,'yyyy/MM/dd'))
```
...
source_df = spark.table('airline_silver.flight_curated')
target = DeltaTable.forName(spark, 'airline_gold.ops_kpi')
(target.alias('t')
...
SELECT 'flight_curated' AS silver_table, 'ops_kpi' AS gold_table,
       (SELECT COUNT(1) FROM airline_silver.flight_curated) AS silver_count,
       (SELECT COUNT(1) FROM airline_gold.ops_kpi) AS gold_count;
```


## Detailed Notebook Playbook

### Notebook `/Projects/airline/01_Bronze/NB_Bronze_flight_event`
- **Purpose:** Execute a scoped medallion responsibility in the `airline` pipeline.
- **Parameters/widgets:** `env`, `load_date`, `run_id`, `watermark`, `rerun_mode` (widget defaults validated at start).
- **Input tables/paths:** `abfss://landing@stairline{env}/flight_event/`.
...
- **Output tables/paths:** `airline_bronze.flight_event` with audit columns `_run_id`, `_adf_pipeline_run_id`, `_processed_ts`.
...
### Notebook `/Projects/airline/02_Silver/NB_Silver_flight_curated`
- **Purpose:** Execute a scoped medallion responsibility in the `airline` pipeline.
- **Parameters/widgets:** `env`, `load_date`, `run_id`, `watermark`, `rerun_mode` (widget defaults validated at start).
- **Input tables/paths:** `airline_bronze.flight_event`.
- **Transformation logic:** Enforce schema, derive surrogate keys, deduplicate by business key + event time, and apply business-rule flags.
- **Output tables/paths:** `airline_silver.flight_curated` with audit columns `_run_id`, `_adf_pipeline_run_id`, `_processed_ts`.
- **Expectations/checks:** Row-count thresholds, null checks on mandatory columns, duplicate ratio threshold, and freshness SLA check.
- **Failure handling:** Write failed records to quarantine path, log exception context to `audit.pipeline_errors`, return non-zero status to ADF for controlled retry/escalation.

### Notebook `/Projects/airline/03_Gold/NB_Gold_ops_kpi`
- **Purpose:** Execute a scoped medallion responsibility in the `airline` pipeline.
- **Parameters/widgets:** `env`, `load_date`, `run_id`, `watermark`, `rerun_mode` (widget defaults validated at start).
- **Input tables/paths:** `airline_silver.flight_curated`.
- **Transformation logic:** Enforce schema, derive surrogate keys, deduplicate by business key + event time, and apply business-rule flags.
- **Output tables/paths:** `airline_gold.ops_kpi` with audit columns `_run_id`, `_adf_pipeline_run_id`, `_processed_ts`.
- **Expectations/checks:** Row-count thresholds, null checks on mandatory columns, duplicate ratio threshold, and freshness SLA check.
- **Failure handling:** Write failed records to quarantine path, log exception context to `audit.pipeline_errors`, return non-zero status to ADF for controlled retry/escalation.

### Notebook `/Projects/airline/04_Validation/NB_Validate_ops_kpi`
- **Purpose:** Execute a scoped medallion responsibility in the `airline` pipeline.
- **Parameters/widgets:** `env`, `load_date`, `run_id`, `watermark`, `rerun_mode` (widget defaults validated at start).
- **Input tables/paths:** `airline_gold.ops_kpi`.
- **Transformation logic:** Enforce schema, derive surrogate keys, deduplicate by business key + event time, and apply business-rule flags.
- **Output tables/paths:** `audit.validation_results` with audit columns `_run_id`, `_adf_pipeline_run_id`, `_processed_ts`.
- **Expectations/checks:** Row-count thresholds, null checks on mandatory columns, duplicate ratio threshold, and freshness SLA check.
- **Failure handling:** Write failed records to quarantine path, log exception context to `audit.pipeline_errors`, return non-zero status to ADF for controlled retry/escalation.

### Notebook `/Projects/airline/05_Recovery/NB_Backfill_flight_event`
- **Purpose:** Execute a scoped medallion responsibility in the `airline` pipeline.
- **Parameters/widgets:** `env`, `load_date`, `run_id`, `watermark`, `rerun_mode` (widget defaults validated at start).
- **Input tables/paths:** `abfss://bronze@stairline{env}/flight_event/`.
- **Transformation logic:** Enforce schema, derive surrogate keys, deduplicate by business key + event time, and apply business-rule flags.
- **Output tables/paths:** `airline_silver.flight_curated` with audit columns `_run_id`, `_adf_pipeline_run_id`, `_processed_ts`.
- **Expectations/checks:** Row-count thresholds, null checks on mandatory columns, duplicate ratio threshold, and freshness SLA check.
- **Failure handling:** Write failed records to quarantine path, log exception context to `audit.pipeline_errors`, return non-zero status to ADF for controlled retry/escalation.
...
> **Reference:** Adapted from [https://github.com/Analytics6/Azure-Data-Engineering-Notes](https://github.com/Analytics6/Azure-Data-Engineering-Notes) — `08-Serverless-Data-Processing-Kubernetes-Orchestration.md, Data_Pipeline_Optimization_Fertilizer_Company.md`

## Project Overview and Business Context

Batch IRROPS (irregular operations) reporting: flight delays, crew, baggage with SLA alerts.

**Karthik focus:** Airline ops batch domain (unique in portfolio); optimized batch pipelines.
...
### Gold
- `gold.fact_flight_delay`
- `gold.irrops_daily_summary`
...
| Pipeline | Purpose |
|----------|---------|
| `PL_KAR_IRROPS_Bronze` | See ADF orchestration below |
| `PL_KAR_IRROPS_Gold` | See ADF orchestration below |
| `PL_KAR_IRROPS_Alerts` | See ADF orchestration below |
...
```
/Karthik/Airline/NB_IRROPS_Aggregates
NB_Delay_Alerts
```
